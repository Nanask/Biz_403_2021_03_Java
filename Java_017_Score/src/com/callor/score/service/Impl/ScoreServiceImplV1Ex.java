...
public class ScoreServiceImplV1Ex extends ScoreServiceImplV1 {
	...
	public ScoreServiceImplV1Ex() {
		...
		/*
		 * menuServiceImplV1 클래스의 생성자는 두개의 매개변수(아규먼트, 파라메터)를 전달하면서
		 * 호출을 해야한다.
		 * 첫번째 매개변수는 프로젝트의 title
		 * 두번째 매개변수는 업무 리스트를 담을 List형 객체
		 * 
		 * title은 단순 문자열이기 때문에 바로 전달할 수 있는데
		 * List형 객체는 List 객체를 생성하고, 데이터를 add 한 후 전달해야 하므로
		 * 약간의 추가 연산 코드가 필요하다.
		 * 
	...
	public void selectMenu() {
		...
		while(true) {
			 ...
			 */
			if(menu == null) {
			//1번을 실행했을 때에는 2번과 3번을 묻지 않는다.
				
			//2번 코드
			/*
			 * 만약 menu변수의 값이 1이면 menu가 1일때 실행되고 코드가 종료되면
			 * else이후 코드는 무시하고 바로 if문이 종료된다.
			 * 
