...
//ScoreVO가 제일 먼저 생성되서 사용할 변수를 설정하자!
 ...
 * 필요할때마다 객체로 생성하고, 데이터를 Setter하고 연산을 수행한 후 데이터를 Getter하는 방식으로 활용한다.
 * 
...
public class ScoreVO {
	...
	//private를 사용하여 정보를 은닉한다.
	private String num;
	private String name;
	
	// VO클래스에 숫자형변수를 선언할 때 primitive 형이 아닌 wrapper 클래스 형으로 선언할때는
	// 0으로 clear를 해 주자.
	// 이러한 불편을 방지하기 위하여 primitive 형으로 만들기도 하지만 null값을 취급하기가 어려운 
	// 단점이 발생한다.
	...
	/*
	 * VO 클래스를 모델링하고 : 추상화
	 * 변수를 private으로 선언하였다 : 정보의은닉
	 * 은닉된 변수에 접근하기 위하여 getter,setter method를 생성
	 * 
	 * getter와 setter를 생성하고 봤더니 총점(total)과 평균(avg)는 점수가 입력되면 항상 계산되어야 하는 항목
	 *  = 외보에서 이 변수에 값을 저장할 일이 별로 없을 것 같다.
	 *  = 외부에서 total, avg 값을 요청하면 그때 그때 계산하여 return하면 될 것같다.
	 *  1. total, avg 변수를 삭제하고
	 *  2. total, avg 변수의 setter method를 제거한다.
	 *  3. total, avg 변수의 getter method를 변경한다.
	 *  과목 총점과 과목평균을 계산하여 return 하도록 한다.
	 */
	 ...
	 * 
	 * 실제코드는 3과목의 총점을 계산하고 계산된 결과를 return하는 구조로 되어 있다.
	 * 
	 ...
	 * 
	 * 그래서 toString() method를 VO객체의 변수에 담긴 값들을 문자열로 만들어 return하는 코드로 다시 작성한다.
