...
public class App_01A {
	...
	public static void main(String[] args) {
		...
		/*
		 * for() 반복문에서 사용할 num(index변수)를 for() 반복문이 시작전에 선언하고 실행을 하면 
		 * for() 반복문이 종료된 후에 num 값을 읽을 수 있다.
		 * 
		 * 이때
		 * 1. 만약 중간에 break를 만나서 for()가 중단되거나
		 * 2. break 없이 for() 반복문이 모두 완료되거나 할텐데 1과 2으 ㅣ경우에 num변수의 상태가 다르다.
		 * 
		 * 보통은 num 변수의 값은 조건문에 최대값과 같다.
		 * 1번의 경우 num 값은 num2보다 항상 작다.
		 * 2번의 경우 num 값은 num2보다 크거나 같은 상황이 된다.
		 * if(num == num2) 라는 비교문을 사용하는 것 보단 if(num < num2) 조건문을 사용하는 것이
		 * 논리적인 문제를 최소화 할 수 있다.
		 * 
		 */
