...
public class Number_02 {
	...
	public static void main(String[] args) {
		...
		//valueof와 같은 표현이므로 valueof를 사용하는것이 더 좋다.
		
		/*
		 * 데이터를 단독으로 처리할때는 큰 문제가 없는데 파일에 저장, 파일에서 읽기
		 * 네트워크(인터넷)등을 통하여 전송(공유) 등으로 데이터가 컴퓨터와 컴퓨터간의 이동할때
		 * 데이터를 효율적으로 오류가 나지 않도록 이동하는 다양한 방법이 있다.
		 * 여러가지 방법이 취하는 많은 공통된 부분에 모든 데이터를 문자열화 하여 전송하는 방법이 있다.
		 * 숫자, 문자 특수기호 등을 모두 문자열(ASCII)코드로 전송하고, 받는 곳에서 사용하기 전에
		 * 적절한 Type으로 변경하여 사용한다.
		 * 
		 * Java에서는 WapperClass.parse*() method나 WrapperClass.ValueOf() method를 사용하여 
		 * 문자열형 숫자를 숫자 값으로 변환시키는 것을 지원한다.
		 * 
		 * 이 과정에서 숫자값으로 변환할 수 없는 문자열이 섞여있는 경우 NumberFormatException이 발생할 수 있다.  
		 */
		intNum1 = Integer.valueOf("100");
		
		intNum1 = Integer.valueOf( "" + 30 );
		intNum1 = Integer.valueOf( 30 + ""); // = "30"
		//숫자와 함께 쓰는 경우에 빈칸이 없다면 문제가 발생하지 않는다
		...
		//""은 null과 유사한 값이다.
		//자바에서는 ""과 null을 완전히 같은 값으로 취급하지 않는다. 
